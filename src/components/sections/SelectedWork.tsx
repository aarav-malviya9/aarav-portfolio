...
function ProjectCard({ 
  ...
  const opacity = useTransform(smoothDistance, [-2, -1, 0, 1, 2], [0, 0.4, 1, 0.4, 0]);
  const blur = useTransform(smoothDistance, [-2, -1, 0, 1, 2], ["blur(10px)", "blur(4px)", "blur(0px)", "blur(4px)", "blur(10px)"]);
  const zIndex = useTransform(smoothDistance, [-2, -1, 0, 1, 2], [0, 10, 50, 10, 0]);
  ...
  return (
    ...
    >
      <div className="w-full h-full relative">
        <div className={`w-full h-full bg-gradient-to-br ${project.gradient} border border-white/5 rounded-2xl p-8 md:p-12 flex flex-col justify-between backdrop-blur-3xl shadow-[0_0_80px_rgba(0,0,0,0.5)]`}>
